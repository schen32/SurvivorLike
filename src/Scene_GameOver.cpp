...
void Scene_GameOver::loadScene()
{
	m_entityManager = EntityManager();

	auto title = m_entityManager.addEntity("ui", "Game Over");
	...
	auto playButton = m_entityManager.addEntity("button", "Restart");
...
void Scene_GameOver::onExitScene()
{
	m_selectedIndex = 0;
	auto& bgm = m_game->assets().getMusic(m_musicName);
	bgm.stop();
...
void Scene_GameOver::onEnterScene()
{
	auto& window = m_game->window();
	window.setView(m_pauseView);
...
void Scene_GameOver::select()
{
	for (auto& button : m_entityManager.getEntities("button"))
	{
		if (!Utils::IsInside(m_mousePos, button)) continue;

		if (button->name() == "Restart" && m_game->changeScene("PLAY", std::make_shared<Scene_Play>(m_game)))
			onExitScene();
